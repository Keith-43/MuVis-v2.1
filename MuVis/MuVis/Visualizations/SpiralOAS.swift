...
struct SpiralOAS: View {
    ...
    var body: some View {
        ZStack {
            if(manager.option==0 || manager.option==2) { SpiralOAS_LayoutBackground() }
            SpiralOAS_Live()
            if(manager.option==0 || manager.option==2) { EllipticalNoteNames() } // struct code in VisUtilities file
        }
    }
}
...
struct SpiralOAS_Live: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .foregroundStyle( manager.option < 2 ?
