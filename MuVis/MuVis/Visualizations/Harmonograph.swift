...
// option0  TriOct MuSpectrum   Lissajous
// option1  TriOct MuSpectrum
// option2  plain               Lissajous
// option3  plain               Lissajous (half-amplitude)
///
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                if(manager.option==0 || manager.option==2 || manager.option==3) {
                    LissajousFigure()
                }
            }
        }
...
struct Harmonograph_DoubleSpectrum : View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            // Make a local copy of the muSpectrum array so not accessing distant array inside a tight loop:
...
// ---------------------------------------------------------------------------------------------------------------------
            ...
            let colorSize: Int = 1000    // This determines the frequency of the color change over time.
            var hue:  Double = 0.0
            ...
            Path { path in
                ...
                Harmonograph_DoubleSpectrum.colorIndex = (Harmonograph_DoubleSpectrum.colorIndex >= colorSize) ?
                                                          0 :
                                                          Harmonograph_DoubleSpectrum.colorIndex + 1
                
                hue = Double(Harmonograph_DoubleSpectrum.colorIndex) / Double(colorSize)          // 0.0 <= hue < 1.0
            }
            .foregroundColor(Color(hue: hue, saturation: 1.0, brightness: 0.9))
...
// ---------------------------------------------------------------------------------------------------------------------
            ...
            .foregroundColor(colorScheme == .dark ? .black : .white) // Toggle between black and white background color.
...
// ---------------------------------------------------------------------------------------------------------------------
            ...
            Path { path in
                ...
                for point in 1 ..< pointsPerRow {
                    ...
                    magY = CGFloat(muSpectrum[pointsPerRow + point]) * halfHeight
                    magY = min(max(0.0, magY), halfHeight)
                    y = halfHeight - magY
                    path.addLine(to: CGPoint(x: x, y: y))
                }
...
struct LissajousFigure : View {
    ...
    static var oldHorAngle: Double = 0.0  // persistent: Maintains phase of horizontal waveform across frames
    static var oldVerAngle: Double = 0.0  // persistent: Maintains phase of vertical waveform across frames
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            ...
            Path { path in
                ...
                peaks = spectralEnhancer.pickPeaks(inputArray: spectrum, peakThreshold: threshold)
...
// --------------------------------------------------------------------------------------------------------------------
                ...
                for binNum in startBin ..< midBin {
                    if (peaks[binNum]) {
                        lowerPeakCount  += 1
                        peakAmplitude = Double( spectrum[binNum] )
...
// ------------------------------------------------------------------------------------------------------------
                ...
                if(lowerPeakCount > 0 && upperPeakCount > 0)    // There is at least one peak in each of the lower and upper 3 octaves.
                {
                    ...
                    for i in 0 ..< dataLength {
                        angle = LissajousFigure.oldHorAngle + ( 2.0 * Double.pi * Double(i) / period )
                        horWaveform[i] = CGFloat(maxPeakAmplitude_L1) * CGFloat( sin(angle) )
                    }
                    LissajousFigure.oldHorAngle = angle

                    period = AudioManager.sampleRate / maxPeakFrequency_U1
                    for i in 0 ..< dataLength {
                        angle = LissajousFigure.oldVerAngle + ( 2.0 * Double.pi * Double(i) / period )
                        verWaveform[i] = CGFloat(maxPeakAmplitude_U1) * CGFloat( sin(angle) )
                    }
                    LissajousFigure.oldVerAngle = angle
                }
        ...
        .background( ( manager.option > 1 ) ? backgroundColor : Color.clear )
